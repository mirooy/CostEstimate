...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute) {
    ...
    while (iter.hasNext()) {
      ...
      if (quantization(rec, attribute) < this.minValue) {
        this.minValue = quantization(rec, attribute);
      }
    }
    ...
    while (iter2.hasNext()) {
      ...
      if (quantization(rec, attribute) > this.maxValue) {
        this.maxValue = quantization(rec, attribute);
      }
    }

    this.width = (this.maxValue - this.minValue) / numBuckets;

    float newMin = this.minValue;
    for(int i = 0 ; i< numBuckets;i++) {
      buckets[i] = new Bucket<>(newMin,newMin+ width);
      newMin = newMin + width;

    }
    Iterator<Record> val = table.iterator();
    while (val.hasNext()) {
      Record rec = val.next();
      if(width == 0) {
        buckets[numBuckets-1].increment(quantization(rec,attribute));

      } else{
        int index = bucketIndex(quantization(rec,attribute));
          buckets[index].increment(quantization(rec,attribute));
  ...
  private float [] allEquality(float qvalue){
    float [] result = new float[this.numBuckets];
    for(int i =0; i < numBuckets; i ++) {
      if(i == bucketIndex(qvalue)){
        if(buckets[i].getDistinctCount() < 1) {
          result[i] = 1;
        } else {
